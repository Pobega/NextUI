...
1. **`fbgl` accepts NextUI's fullscreen GLES window** — the one real
   unknown in the video chain (04). Everything downstream (shaders,
   overlays) is proven on this GPU by tg5040.
2. **ES shim handoff** — S31 launching our fake
   `emulationstation-standalone`, updater platform detection
   (`sun50iw10`), installer flow, launch loop.
